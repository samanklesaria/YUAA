...
-(void)showDataSource: (StatPoint *)stat named: (NSString *)name {
    ...
    plotSpace.xRange = [CPTPlotRange plotRangeWithLocation:CPTDecimalFromDouble(backupx) length:CPTDecimalFromDouble(xrange)];
    plotSpace.yRange = [CPTPlotRange plotRangeWithLocation:CPTDecimalFromDouble(backupy) length:CPTDecimalFromDouble(yrange)];
    ...
    NSArray *yexclusionRanges = [NSArray arrayWithObjects:
                                 [CPTPlotRange plotRangeWithLocation:CPTDecimalFromFloat(minval) length:CPTDecimalFromFloat(-1*backupy)], 
                                 [CPTPlotRange plotRangeWithLocation:CPTDecimalFromFloat(minval + backupy) length:CPTDecimalFromFloat(minval)],
                                 nil];
	y.labelExclusionRanges = yexclusionRanges;
    ...
    for (i = 0; i < [colorArray count]; i++) {
        ...
        CPTMutableLineStyle *symbolLineStyle = [CPTMutableLineStyle lineStyle];
        symbolLineStyle.lineColor = [colorArray objectAtIndex: i];
...
// bay points are arrays of times
// we need a map from times to x y dictionaries in addition to the arrays
-(NSNumber *)numberForPlot:(CPTPlot *)plot field:(NSUInteger)fieldEnum recordIndex:(NSUInteger)index 
{
    SharedData *s = [SharedData instance];
    int idx = [(NSNumber *)plot.identifier intValue];
    switch (idx) {
        case -1: [[dataForPlot.points objectAtIndex: index] valueForKey:(fieldEnum == CPTScatterPlotFieldX ? @"x" : @"y")];
        case 0: [NSNumber numberWithInt: [[dataForPlot.bayNumToPoints objectForKey: [s.bayOpenData objectAtIndex: index]] intValue]];
        case 1: [NSNumber numberWithInt: [[dataForPlot.bayNumToPoints objectForKey: [s.bayCloseData objectAtIndex: index]] intValue]];
    }
